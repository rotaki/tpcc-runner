...
#include "protocols/common/localset_tracker.hpp"
...
// Serialization Test
template <>
class Silo<MasstreeIndexes<ValueTest>> {
public:
    using Index = MasstreeIndexes<ValueTest>;
    using Key = typename Index::Key;
    using Value = typename Index::Value;
    using NodeInfo = typename Index::NodeInfo;
    using LeafNodeID = typename Index::LeafNodeID;
    using RWNodeTable = typename Index::RWNodeTable;
    ...
    const Rec* read(TableID table_id, Key key) {
        ...
        if (rw_iter == rw_table.end()) {
            ...
            typename Index::Result res = idx.find(table_id, key, val, nm);
            if (res == Index::Result::NOT_FOUND) return nullptr;
            // Read record pointer, tidword, and txid from index
            Rec* rec = nullptr;
            TidWord tw;
            TxID txid;
            get_record_pointer(*val, rec, tw, txid);
            // Null check
            if (!is_readable(tw)) return nullptr;
            rw_table.emplace_hint(
                rw_iter, std::piecewise_construct, std::forward_as_tuple(key),
                std::forward_as_tuple(nullptr, tw, ReadWriteType::READ, false, val, txid));
            return rec;
        }
        ...
        if (rwt == ReadWriteType::READ) {
            ...
            TidWord tw;
            TxID txid;
            get_record_pointer(*(rw_iter->second.val), rec, tw, txid);
            if (!is_same(tw, rw_iter->second.tw)) return nullptr;
            return rec;
        } else if (rwt == ReadWriteType::UPDATE || rwt == ReadWriteType::INSERT) {
            if (!is_tidword_latest(*(rw_iter->second.val), rw_iter->second.tw)) return nullptr;
    ...
    Rec* insert(TableID table_id, Key key) {
        ...
        if (rw_iter == rw_table.end()) {
            ...
            new_val->tidword.latest = 1;  // exist in index
            new_val->tidword.absent = 1;  // cannot be seen by others
            new_val->txid.id = 0;         // insert with null id (will be updated in precommit)
            res = idx.insert(table_id, key, new_val, nm, rwns[table_id]);
            if (res == Index::Result::NOT_INSERTED) {
                MemoryAllocator::deallocate(new_val);
                return nullptr;  // abort
            }
            ...
            auto new_iter = rw_table.emplace_hint(
                rw_iter, std::piecewise_construct, std::forward_as_tuple(key),
                std::forward_as_tuple(
                    rec, new_val->tidword, ReadWriteType::INSERT, true, new_val, new_val->txid));
    ...
    Rec* update(TableID table_id, Key key) {
        ...
        if (rw_iter == rw_table.end()) {
            ...
            TidWord tw;
            TxID txid;
            copy_record(*val, rec, tw, txid, record_size);
            // Null check
            if (!is_readable(tw)) {
                MemoryAllocator::deallocate(rec);
                return nullptr;
            ...
            auto new_iter = rw_table.emplace_hint(
                rw_iter, std::piecewise_construct, std::forward_as_tuple(key),
                std::forward_as_tuple(rec, tw, ReadWriteType::UPDATE, false, val, txid));
            // Place it in write set
            auto& w_table = ws.get_table(table_id);
            w_table.emplace_back(key, new_iter);
            return rec;
        ...
        if (rwt == ReadWriteType::READ) {
            ...
            Rec* rec = MemoryAllocator::aligned_allocate(record_size);
            TidWord tw;
            TxID txid;  // insert with null id (will be updated in precommit)
            copy_record(*rw_iter->second.val, rec, tw, txid, record_size);
            // Check if tidword stored locally is the latest
            if (!is_same(tw, rw_iter->second.tw)) {
                MemoryAllocator::deallocate(rec);
                return nullptr;
    ...
    bool read_scan(
        ...
        typename Index::Result res;
        if (reverse) {
            res = idx.get_kv_in_rev_range(table_id, lkey, rkey, count, kv_map, nm, rwns[table_id]);
        } else {
            res = idx.get_kv_in_range(table_id, lkey, rkey, count, kv_map, nm, rwns[table_id]);
        }
        if (res == Index::Result::BAD_SCAN) return false;
    ...
    Rec* remove(TableID table_id, Key key) {
        ...
        if (rw_iter == rw_table.end()) {
            ...
            auto new_iter = rw_table.emplace_hint(
                rw_iter, std::piecewise_construct, std::forward_as_tuple(key),
                std::forward_as_tuple(nullptr, tw, ReadWriteType::DELETE, false, val, txid));
    ...
    bool precommit() {
        ...
        for (TableID table_id: tables) {
            ...
            for (auto w_iter = w_table.begin(); w_iter != w_table.end(); ++w_iter) {
                ...
                auto rwt = rw_iter->second.rwt;
                Rec* old = exchange(rw_iter->second.val->rec, rw_iter->second.rec);
                exchange(rw_iter->second.val->txid.id, this_txid.id);
                TidWord new_tw;
                new_tw.epoch = commit_tw.epoch;
                new_tw.tid = commit_tw.tid;
                new_tw.latest = !(rwt == ReadWriteType::DELETE);
                ...
                GarbageCollector::collect(commit_tw.epoch, old);
                if (rwt == ReadWriteType::DELETE) {
                    idx.remove(table_id, w_iter->first, rwns[table_id]);
                    GarbageCollector::collect(commit_tw.epoch, rw_iter->second.val);
                }
            }
        }

        // Track ReadSet and WriteSet for test
        for (TableID table_id: tables) {
            auto& rw_table = rws.get_table(table_id);
            auto& lst = ThreadLocalRWTracker<Key>::get_table(this_txid);
            for (auto rw_iter = rw_table.begin(); rw_iter != rw_table.end(); ++rw_iter) {
                auto rwt = rw_iter->second.rwt;
                if (rwt == ReadWriteType::INSERT)
                    continue;
                else if (rwt == ReadWriteType::READ)
                    lst.emplace_back(
                        OperationType::R, table_id, rw_iter->first, rw_iter->second.txid);
                else if (rwt == ReadWriteType::UPDATE || rwt == ReadWriteType::DELETE)
                    lst.emplace_back(
                        OperationType::W, table_id, rw_iter->first, rw_iter->second.txid);
            }
        }

        // Track NodeSet for test
        for (TableID table_id: tables) {
            auto& rwnt = rwns[table_id];
            auto& lnst = ThreadLocalNodeTracker<Index>::get_table(this_txid);
            for (auto rwn_iter = rwnt.begin(); rwn_iter != rwnt.end(); ++rwn_iter) {
                if (rwn_iter->first)
                    lnst.emplace_back(
                        table_id, static_cast<uint32_t>(commit_tw.epoch),
                        reinterpret_cast<uintptr_t>(rwn_iter->first), rwn_iter->second.v_read,
                        std::move(rwn_iter->second.v_writes));
            }
        }
...
private:
    ...
    WriteSet<Key, Value> ws;
    NodeSet ns;
    std::unordered_map<TableID, RWNodeTable> rwns;

    void get_record_pointer(Value& val, Rec*& rec, TidWord& tw, TxID& txid) {
        TidWord expected;
        expected.obj = load_acquire(val.tidword.obj);
        while (true) {
            // loop while locked
            ...
            // read record and tidword
            rec = load_acquire(val.rec);  // val.rec could be nullptr
            txid.id = load_acquire(val.txid.id);
            // DO NOT REORDER
            // TidWord need to be loaded last
            tw.obj = load_acquire(val.tidword.obj);
    ...
    void copy_record(Value& val, Rec* rec, TidWord& tw, TxID& txid, uint64_t rec_size) {
        if (rec == nullptr) throw std::runtime_error("memory not allocated");
        TidWord expected;
        expected.obj = load_acquire(val.tidword.obj);
        while (true) {
