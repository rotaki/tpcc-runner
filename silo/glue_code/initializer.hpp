...
inline void insert_into_index(TableID table_id, uint64_t key, void* rec) {
    TidWord tw;
    tw.lock = 0;
    tw.latest = 1;
    tw.absent = 0;
...
inline void create_and_insert_district_record(uint16_t d_w_id, uint8_t d_id) {
    ...
    District* d = reinterpret_cast<District*>(MemoryAllocator::aligned_allocate(sizeof(District)));
...
inline void create_and_insert_customer_record(
    ...
    Customer* c = reinterpret_cast<Customer*>(MemoryAllocator::aligned_allocate(sizeof(Customer)));
...
inline std::pair<Timestamp, uint8_t> create_and_insert_order_record(
    ...
    insert_into_index(get_id<OrderSecondary>(), os_key.get_raw_key(), reinterpret_cast<void*>(os));
...
inline void create_and_insert_neworder_record(uint16_t no_w_id, uint8_t no_d_id, uint32_t no_o_id) {
    ...
    NewOrder* no = reinterpret_cast<NewOrder*>(MemoryAllocator::aligned_allocate(sizeof(NewOrder)));
