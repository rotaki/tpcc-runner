...
NodeMap& NodeSet::get_nodemap(TableID table_id) {
    return ns[table_id];
}

Silo::Silo(uint32_t epoch)
...
const Rec* Silo::read(TableID table_id, Key key) {
...
Rec* Silo::insert(TableID table_id, Key key) {
    ...
    if (rs_iter == rs_table.end() && ws_iter == ws_table.end()) {
        ...
        Value* val;
        Index::Result res = idx.find(table_id, key, val);
...
Rec* Silo::update(TableID table_id, Key key) {
...
bool Silo::read_scan(
    ...
    LOG_INFO(
        "READ_SCAN (e: %u, t: %lu, lk: %lu, rk: %lu, c: %ld)", starting_epoch, table_id, lkey, rkey,
        count);
    const Schema& sch = Schema::get_schema();
    Index& idx = Index::get_index();
...
Rec* Silo::remove(TableID table_id, Key key) {
...
bool Silo::precommit() {
    ...
    commit_tw.epoch = load_acquire(EpochManager::get_global_epoch());
    ...
    for (TableID table_id: tables) {
        ...
        for (auto iter = nm.begin(); iter != nm.end(); ++iter) {
            ...
            if (iter->second != current_version) {
                LOG_DEBUG(
                    "NODE VERIFY FAILED (t: %lu, old_v: %lu, new_v: %lu)", table_id, iter->second,
                    current_version);
                unlock_writeset();
                return false;
            }
...
void Silo::read_record_and_tidword(Value& val, Rec* rec, TidWord& tw, size_t rec_size) {
...
void Silo::read_tidword(Value& val, TidWord& tw) {
    TidWord expected, check;
    expected.obj = load_acquire(val.tidword.obj);
    while (true) {
        while (expected.lock) {
...
void Silo::unlock_writeset() {
    unlock_writeset(UINT64_MAX, UINT64_MAX);
}

void Silo::unlock_writeset(TableID end_table_id, Key end_key) {
    for (TableID table_id: tables) {
        auto& ws_table = ws.get_table(table_id);
        auto& vs_table = vs.get_table(table_id);
        for (auto w_iter = ws_table.begin(); w_iter != ws_table.end(); ++w_iter) {
...
bool Silo::is_readable(const TidWord& tw) {
    return !tw.absent && tw.latest;
}

bool Silo::is_valid(const TidWord& current, const TidWord& expected) {
    return current.epoch == expected.epoch && current.tid == expected.tid && is_readable(current);
}
